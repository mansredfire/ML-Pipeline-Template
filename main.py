...
@click.group()
def cli():
    """ML Pipeline - ML Pipeline Template CLI"""
    pass

@cli.command()
@click.option('--domain', '-d', required=True, help='Stock ticker (e.g. AAPL)')
...
def analyze(domain, output, format, verbose):
    ...
    console.print(f"\n[bold cyan]🎯 ML Pipeline Template - Analyzing {domain}[/bold cyan]\n")
    
    with Progress() as progress:
        # Detect technology stack
        task1 = progress.add_task("[cyan]Preparing company info...", total=100)
        tech_info = {'technologies': [], 'endpoints': ['/'], 'has_auth': False}
...
def _display_results(results, format_type, output_file, verbose):
    ...
    if results['chain_predictions']:
        console.print("\n[bold]⛓️  Potential Pattern Chains[/bold]\n")
        for chain in results['chain_predictions']:
            console.print(f"  • {chain['name']}")
            console.print(f"    Types: {' → '.join(chain['types'])}")
