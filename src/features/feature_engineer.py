"""Feature engineering for data records"""
...
class FeatureEngineer:
    """Engineer features from data records for ML models"""
    ...
    def fit_transform(self, reports: List[DataRecord]) -> pd.DataFrame:
        """
        Transform data records into feature DataFrame
    ...
    def _extract_features(self, report: DataRecord) -> Dict[str, Any]:
        ...
        features['priority_score'] = getattr(report, 'priority_score', 0.0)
        features['reward_amount'] = getattr(report, 'reward_amount', 0.0)
        features['source_quality'] = getattr(report, 'source_quality', 0)
        ...
        features['record_type'] = getattr(report, 'record_type', 'Unknown')
        ...
        features['category'] = getattr(report, 'category', 'Unknown')
        features['http_method'] = getattr(report, 'http_method', 'Unknown')
        features['domain_category'] = getattr(report, 'domain_category', 'Unknown')
        features['category_id'] = getattr(report, 'category_id', 'Unknown')
        ...
        # Risk and complexity scores (if enriched)
        features['risk_score'] = getattr(report, 'risk_score', 0.0)
        features['complexity_score'] = getattr(report, 'complexity_score', 0.0)
    ...
    def _encode_categorical(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        categorical_columns = [
            'record_type', 'platform', 'target_company', 
            'target_domain', 'target_program', 'category',
            'http_method', 'domain_category', 'category_id', 'tech_stack'
        ]
    ...
    def transform(self, reports: List[DataRecord]) -> pd.DataFrame:
        """
